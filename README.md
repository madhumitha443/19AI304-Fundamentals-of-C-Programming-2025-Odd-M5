...
# Program:
```
#include <stdio.h> void swapByValue(int a, int b) { int temp = a; a = b; b = temp; printf("Inside swapByValue: a = %d, b = %d\n", a, b); }

void swapByReference(int *a, int *b) { int temp = *a; *a = *b; *b = temp; }

int main() { int x, y; scanf("%d %d", &x, &y);
...
# Program:
```
#include <stdio.h> int fibonacci(int n) { if (n == 1) return 0;
else if (n == 2) return 1;
else return fibonacci(n - 1) + fibonacci(n - 2); }

int main() { int n; scanf("%d", &n); if (n <= 0) { printf("Please enter a positive integer.\n"); return 1; } printf("First %d terms of Fibonacci series: ", n); for (int i = 1; i <= n; i++) { printf("%d ", fibonacci(i)); } printf("\n");
...
# Program:
```
#include <stdio.h> void printSequence(int current, int upper) { if (current > upper) return;
...
int main() { int lower, upper; scanf("%d", &lower); scanf("%d", &upper);
...
int main() { int n, i, sum = 0; int *arr; scanf("%d", &n); arr = (int *)calloc(n, sizeof(int)); if (arr == NULL) { printf("Memory allocation failed.\n"); return 1; } for (i = 0; i < n; i++) { scanf("%d", &arr[i]); } for (i = 0; i < n; i++) { sum += arr[i]; }
...
# Program:
```
#include <stdio.h> void displayArray(int arr[], int size) { printf("Array elements are: "); for(int i = 0; i < size; i++) { printf("%d ", arr[i]); } printf("\n"); }
